...
#
# srun launches pd_dis_chat.py on BOTH nodes simultaneously.  Each node detects
...
#
# Dataset options (forwarded to pd_dis_chat.py):
...
#SBATCH --job-name=pd_disagg_chat
...
# fp8 KV-cache dtype triggers flashinfer kernel JIT compilation; the cache
...
# HF `datasets` needs to write lock files at load time. When HF_HOME points
# to the shared (read-only) zf28 cache, redirect the datasets cache to a
# user-writable path so load_dataset() for AIME25 / LVEval succeeds.
# Model weights cache stays under HF_HOME (read-only reuse is fine).
_USER_DATASETS_CACHE="/scratch/$USER/comp529/cache/datasets"
mkdir -p "$_USER_DATASETS_CACHE" 2>/dev/null || true
export HF_DATASETS_CACHE="$_USER_DATASETS_CACHE"

# Older `datasets` copies dynamic loading scripts (e.g. LVEval.py) into
# $HF_HOME/modules at trust_remote_code=True. Our HF_HOME points at zf28's
# shared cache which is read-only for hx50. Redirect the modules cache to
# a user-writable location.
_USER_MODULES_CACHE="/scratch/$USER/comp529/cache/modules"
mkdir -p "$_USER_MODULES_CACHE" 2>/dev/null || true
export HF_MODULES_CACHE="$_USER_MODULES_CACHE"

# Pinned older `datasets` (2.21.0) for LVEval's script-based loader.
# The shared env's datasets==4.8.4 refuses to run any dataset script, which
# breaks LVEval. Prepend our user-scratch install to PYTHONPATH so that
# pd_dis_chat.py's import sees the older version.
_USER_PYPKGS="/scratch/$USER/comp529/pypkgs"
if [[ -d "$_USER_PYPKGS/datasets" ]]; then
    export PYTHONPATH="$_USER_PYPKGS:${PYTHONPATH:-}"
fi
...
# Export so pd_dis_chat.py can read it on both nodes.
# ---------------------------------------------------------------------------
export PREFILL_HOST=$(scontrol show hostnames "$SLURM_JOB_NODELIST" | head -n 1)

# ---------------------------------------------------------------------------
# srun launches pd_dis_chat.py on every node (SLURM_PROCID=0 on node 0,
...
SCRIPT_PATH="${SCRIPT_DIR}/pd_dis_chat.py"

if [[ ! -f "${SCRIPT_PATH}" ]]; then
	echo "[ERROR] Cannot find ${SCRIPT_PATH}" >&2
	echo "        Submit from the directory containing pd_dis_chat.py, or update SCRIPT_DIR." >&2
	exit 1
fi
